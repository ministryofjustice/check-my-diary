...
describe('azureAppInsights', () => {
  describe('addUserDataToRequests', () => {
    it('adds user data to properties when present', () => {
      ...
      // @ts-expect-error properties isn't defined in the type definition
    ...
    it('handles absent user data', () => {
      ...
      addUserDataToRequests(envelope, createContext(undefined as unknown as HmppsUser))
    ...
    it('handles when no properties have been set', () => {
      const envelope = createEnvelope(undefined as unknown as Record<string, string | boolean>)
