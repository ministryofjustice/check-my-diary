...
async function oauthTokenRequest(oauthRequest, logger) {
  const oauthResult = await getOauthToken(oauthRequest, logger)
...
function getOauthToken(requestSpec, logger) {
...
function signInService(logger) {
  return {
    ...
    async getRefreshedToken(user) {
      ...
      const { token, refreshToken, expiresIn } = await oauthTokenRequest(oauthRequest, logger)
