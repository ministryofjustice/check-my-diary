...
context('A staff member can view their overtime calendar', () => {
  ...
  it('A staff member can view their calendar', () => {
    ...
    const monthDifference =  moment([now.getFullYear(), `0${now.getMonth() + 1}`.slice(-2), '01'].join('-')).diff(new Date('2020-02-01'), 'months', true);

    for(let x = 1; x <= monthDifference; x+=1 ){
      const month = calendarOvertimePage.previousMonth(moment().subtract(x,'months').startOf('month').format('YYYY-MM-DD'))
      month.click()
    }
  ...
  it('A staff member can drill into a overtime day shift', () => {
    ...
    calendarDetailPage.detailStart().should('contain', '07:30 - 12:30').should('contain', 'Start of shift').should('contain', 'Training - Internal')    
